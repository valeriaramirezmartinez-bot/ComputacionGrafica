...
// Variables que serviran para manejar la rotaciòn y velocidad de este
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_B])
	{
		...
		AnimDog = true;     // activa animación de patas, cabeza y cola
		dogPos = glm::vec3(0.0f, 0.0f, 0.0f);  // reinicia posición
		dogRot = 0.0f;
...
void Animation() {
	...
	case 6: // Último giro a la izquierda
	{
		float rotDiff = dogRot - targetRot;
		if (rotDiff > 0.001f) {
			dogRot -= rotSpeed;
	...
	case 7: // Regresa al punto inicial en diagonal sin bucle
	{
		...
		if (length > moveSpeed) { 
			...
			// actualizar rotación solo mientras se mueve
			dogRot = glm::degrees(atan2(direction.x, direction.z));
		}
		else {
			dogPos = targetPos;   
			dogAnim = 0;          
			AnimDog = false;      // detener animación
